...
网关自身**不读任何代理配置**也能正常工作 —— 没设代理就连直连。下面这张表是实测结果（不是推断）：

| 你的环境 | 远程上游（DeepSeek/OpenAI） | 本机上游（vLLM/Ollama） | 要做什么 |
|---|---|---|---|
| 没挂代理 | 直连 ✅ | 直连 ✅ | **什么都不用做** |
| 挂了代理，已设 `HTTP(S)_PROXY` 环境变量 | 走代理 ✅ | **直连**（自动，见下） | **什么都不用做** |
| 挂了代理，只在 Windows 系统设置里配的 | 直连 ⚠️ 可能失败 | 直连 ✅ | 见下方说明 |

### 关于 Windows 系统代理

Go 的 HTTP 客户端**只读 `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` 环境变量，不读 Windows「Internet 选项」里的系统代理**。

所以如果你是通过 Clash / v2ray 之类的**系统代理开关**挂的代理，而没有设环境变量，那么网关访问远程上游时会**直连**：

- 如果你的网络本身能直连上游 → 没问题，不用管
- 如果必须走代理才能连上 → 需要显式设环境变量：
...
Go 的标准库对 `127.0.0.1`、`localhost`、`::1` **默认就绕过代理**，即使你设了 `HTTP_PROXY` 也不会把回环请求发给代理。所以本地 vLLM / Ollama 不会被代理拦掉。

`run.ps1` 会额外把这些地址写进 `NO_PROXY`（并保留你已有的值），作为双保险 —— 对没有代理的环境也没有副作用。

### `run.ps1` 的 `-Proxy` 参数

**默认不用管。** 只有在"有代理、但没导出成环境变量"时才需要：

```powershell
.\run.ps1 -Proxy http://127.0.0.1:7890   # 临时指定
.\run.ps1 -Proxy ""                      # 强制直连（忽略环境变量里的代理）
```

上面表格里的行为都已经过测试验证，见 `internal/proxy` 与 `tools/` 下的验证工具。
